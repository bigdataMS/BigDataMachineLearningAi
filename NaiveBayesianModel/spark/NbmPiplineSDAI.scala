 ...
 * @Desc
 */
object NbmPiplineSDAI {
  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("NbmPiplineSDAI")
    /*.setMaster("local[*]")*/
    ...
    val nmb = new NaiveBayes().
      setLabelCol( "indexedLabel" ).
      setFeaturesCol( "indexedFeatures" )
    //组装pipline
    val  pipeline  =  new  Pipeline().setStages(Array(labelIndexer, featureIndexer, nmb,labelConverter))
