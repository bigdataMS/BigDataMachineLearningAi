...
/**
 * Create by manshu on 2019/6/14 13:53
 *
 */
object GraphTest {
  def main(args: Array[String]): Unit = {
    ...
    //val data=sc.sql(s"select customer_no, FOUR_DEP_CUST_SIZE_LEVEL,BEGIN_DATE,END_DATE from  $tab")
    //data.map(_.toSeq.toArray).collect
    ...
    val vertexRDD: RDD[(Long, (String, Int))] = sc.sparkContext.parallelize(vertexArray)
    val edgeRDD: RDD[Edge[Int]] = sc.sparkContext.parallelize(edgeArray)
    //构造图 Graph[VD,ED]
   ...
   // val vertex=data.rdd.map{case (a,b,c)=> Array(a,(b,c))}
    //data.rdd.map{case Row(a,b,c)=> println(s"$a,$b,$c")}
    //val edge=data.rdd.map{case (a,b,c)=> Edge(a,b,c)}

    graph.vertices.filter { x=> x._2._2>30}.foreach(x=>println(x._2._1+" is "+x._2._2))
    //graph.vertices.filter { x=> x._2._2>30}.foreach(x=>println(s"${x._2._1} is ${x._2._2}"))
